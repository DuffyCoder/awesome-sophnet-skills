...
Before running any Python command below, initialize the skill-local environment:
`bash scripts/ensure_uv_env.sh`

If you need to fill out a PDF form, first check to see if the PDF has fillable form fields. Run this script from this file's directory:
 `uv run --project . python scripts/check_fillable_fields.py <file.pdf>`, and depending on the result go to either the "Fillable fields" or "Non-fillable fields" and follow those instructions.

# Fillable fields
If the PDF has fillable form fields:
- Run this script from this file's directory: `uv run --project . python scripts/extract_form_field_info.py <input.pdf> <field_info.json>`. It will create a JSON file with a list of fields in this format:
```
[
  {
...
`uv run --project . python scripts/convert_pdf_to_images.py <file.pdf> <output_directory>`
...
`uv run --project . python scripts/fill_fillable_fields.py <input pdf> <field_values.json> <output pdf>`
...
`uv run --project . python scripts/extract_form_structure.py <input.pdf> form_structure.json`
...
`uv run --project . python scripts/check_bounding_boxes.py fields.json`
...
`uv run --project . python scripts/convert_pdf_to_images.py <input.pdf> <images_dir/>`
...
`uv run --project . python scripts/fill_pdf_form_with_annotations.py <input.pdf> fields.json <output.pdf>`

## Step 4: Verify Output

Convert the filled PDF to images and verify text placement:
`uv run --project . python scripts/convert_pdf_to_images.py <output.pdf> <verify_images/>`
