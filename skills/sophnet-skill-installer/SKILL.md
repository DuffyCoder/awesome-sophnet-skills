  ...
  moltbot:
    ...
    install:
      - id: install-skills
        ...
        command: ["python3", "scripts/install-skills.py"]
---
...
- Installs into `<workspace>/skills/<skill-name>` (defaults to `~/.openclaw/workspace/skills`).
...
- Installed annotations come from `<workspace>/skills`.
