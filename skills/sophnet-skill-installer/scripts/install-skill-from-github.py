...
def _tmp_root() -> str:
    base = os.path.join(tempfile.gettempdir(), "sophnet")
    os.makedirs(base, exist_ok=True)
    return base


def _request(url: str) -> bytes:
    return github_request(url, "sophnet-skill-install")
...
def _prepare_destination(dest_dir: str, replace: bool) -> None:
    if not os.path.exists(dest_dir):
        return
    if not replace:
        raise InstallError(
            f"Destination already exists: {dest_dir}. Use --replace to overwrite."
        )
    if not os.path.isdir(dest_dir):
        raise InstallError(f"Destination is not a directory: {dest_dir}")
...
def _parse_args(argv: list[str]) -> Args:
    ...
    parser.add_argument(
        "--replace",
        action="store_true",
        help="Replace existing destination skill directories (used for updates).",
    )
    ...
    try:
        ...
        try:
            ...
            for path in source.paths:
                ...
                _prepare_destination(dest_dir, args.replace)
