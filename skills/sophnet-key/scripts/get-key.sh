...
#
# sophnet-key 配置脚本
# 从 Moltbot 配置文件获取 SophNet API Key 或提示用户设置环境变量
#
...
main() {
    ...
    if [[ -n "$api_key" ]]; then
        if [[ "$QUIET" != true ]]; then
            log_success "从配置文件找到 SophNet API Key"
            echo -n "  值: "
    ...
    # 未找到配置
    log_error "未找到 SophNet API Key 配置"
    show_config_help
    return 1
}
