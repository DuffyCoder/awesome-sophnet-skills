#!/usr/bin/env bash
#
# sophnet-key setup script
# Help users configure SophNet API Key environment variable
#
...
# Get SophNet API Key from config
...
add_to_shell_rc() {
    ...
    echo "" >> "$shell_rc"
    echo "# SophNet API Key (added by sophnet-key skill)" >> "$shell_rc"
...
main() {
    echo ""
    echo -e "${blue}=== SophNet API Key Setup ===${nc}"
    echo ""
    ...
    if [[ -n "$api_key" ]]; then
        log_success "Found SophNet API Key"  
        echo -n "  Value: "
        ...
        log_warning "SophNet API Key not found in config"
        echo ""
    fi
    ...
    if [[ -z "$api_key" ]]; then
        ...
        read -s "Enter your SophNet API Key: " api_key
