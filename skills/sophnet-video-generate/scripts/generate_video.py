...
def get_api_key() -> str:
    """Get API key from environment or sophon-key script."""
    # Check environment first
    api_key = os.environ.get("SOPH_API_KEY")
    if api_key:
        return api_key

    # Try to get from sophon-key script - check multiple locations
    possible_key_script_paths = [
        # From app skills directory
        Path("/app/skills/sophon-key/scripts/get-key.sh"),
        # From workspace skills directory
        Path(__file__).parent.parent.parent / "sophon-key" / "scripts" / "get-key.sh",
    ]

    for key_script in possible_key_script_paths:
        if key_script.exists():
            try:
                import subprocess
                result = subprocess.run(
                    ["bash", str(key_script), "--output-quiet"],
                    capture_output=True,
                    text=True,
                    check=True
                )
    ...
    raise ValueError("No API key found. Please set SOPH_API_KEY environment variable.")
